...
pub fn main() !void {
    ...
    if (std.mem.eql(u8, "pack", command)) {
        ...
        const symlink_target = std.fs.cwd().readLink(argument, &symlink_buffer);
        const is_symlink = if (symlink_target) |_| true else |_| false;
        const stat = if (!is_symlink) try std.fs.cwd().statFile(argument) else null;
        if (is_symlink) {
            var archive = try narser.NarArchive.fromSymlink(allocator, symlink_target catch unreachable);
            defer archive.deinit();

            try archive.dump(writer);
        } else switch (stat.?.kind) {
