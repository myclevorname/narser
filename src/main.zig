...
pub fn main(juice: std.process.Init.Minimal) !void {
    ...
    var fw = stdout.writer(io, &stdout_buffer);
    ...
    if (std.mem.eql(u8, "pack", command) or std.mem.eql(u8, "hash", command)) {
        ...
        if (std.mem.eql(u8, "-", argument)) {
            ...
            const stat = try std.Io.Dir.cwd().statFile(io, argument, .{ .follow_symlinks = false });
            switch (stat.kind) {
                .sym_link => {
                    var symlink_buffer: [std.fs.max_path_bytes]u8 = undefined;
                    const size = try std.Io.Dir.cwd().readLink(io, argument, &symlink_buffer);
                    try narser.NixArchive.packSymlink(symlink_buffer[0..size], used_writer);
                },
                .directory => {
                    var dir = try std.Io.Dir.cwd().openDir(io, argument, .{
                        .iterate = true,
                    ...
                    var file = try std.Io.Dir.cwd().openFile(io, argument, .{ .follow_symlinks = false });
                    defer file.close(io);
        ...
        switch (kind) {
            .directory => {
                try std.Io.Dir.cwd().createDir(io, target_path orelse ".", .default_dir);
                var dir = try std.Io.Dir.cwd().openDir(io, target_path orelse ".", .{});
                defer dir.close(io);
            ...
            .file => if (target_path == null or std.mem.eql(u8, "-", target_path.?)) {
                ...
                try out.setPermissions(io, .fromMode(if (exec) 0o777 else 0o666));
            },
